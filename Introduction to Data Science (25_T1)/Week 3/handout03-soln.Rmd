...
uk_gdp_and_weeklyWage %>% 
  ...
  summarise(mean_GDP = mean(GDP_m), std_GDP = sd(GDP_m),
            mean_wage = mean(Weekly_pay), std_wage = sd(Weekly_pay),
...
To address the issue(s), adjustment in the above code chunk can be done by different 
ways. For example, `filter` can be added to implement that in the Exercise 2.
...
*Exercise 3*
```{r Exercise 3}
uk_gdp_and_weeklyWage %>% arrange(GDP_growth_rate) %>%
  head(10)
```
...
uk_gdp_and_weeklyWage %>% 
    summarise(mean_wage = mean(Weekly_pay), mean_GDP = mean(GDP_m), 
              mean_GDP_growth = mean(GDP_growth_rate, na.rm=T))
...
GDP_growth_rate_median <- parse_number(GDP_growth_rate_summary[3,4], 
